export const migrations = [
  ...
  `,
  `
  -- FIX producao (04/07/2026): a primeira migracao de defeitos foi inserida no
  -- meio da lista, mas producao versiona migracoes por posicao e so executa
  -- novas entradas no final. Repetimos a criacao de forma idempotente para
  -- garantir que bancos ja em version >= 62 criem as tabelas do snapshot.
  CREATE TABLE IF NOT EXISTS customer_defect_snapshots (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_file_id UUID REFERENCES source_files(id) ON DELETE SET NULL,
    source_file_path TEXT NOT NULL,
