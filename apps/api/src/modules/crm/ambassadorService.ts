...
export async function getAmbassadorOverview(): Promise<AmbassadorResponse> {
  ...
  const summary: AmbassadorSummary = {
    ...
    statusCounts: {
      ...
      NEW: Number(summaryRow.new_count ?? 0),
    },
  ...
  const ambassadors: AmbassadorListItem[] = ambassadorRows.rows.map((row) => ({
    ...
    avgDaysBetweenOrders: row.avg_days_between_orders === null || row.avg_days_between_orders === undefined ? null : Number(row.avg_days_between_orders),
