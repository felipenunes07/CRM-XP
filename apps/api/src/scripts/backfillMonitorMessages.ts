...
async function main() {
  ...
  try {
    ...
    const activitiesResult = await client.query(`
      WITH raw_activities AS (
        ...
        LEFT JOIN LATERAL (
          ...
          WHERE da.metadata ->> 'senderJid' IS NOT NULL
            AND wpp_inner.participant_jid = da.metadata ->> 'senderJid'
    ...
    const incomingRemoteResult = await client.query(`
      WITH raw_incoming_remote AS (
        ...
        LEFT JOIN LATERAL (
          ...
          FROM whatsapp_participant_profiles wpp_inner
          WHERE wim.participant_jid IS NOT NULL
            AND wpp_inner.participant_jid = wim.participant_jid
    ...
    const incomingParticipantResult = await client.query(`
    ...
    const incomingAliasResult = await client.query(`
