...
import { rebuildReadModels } from "../modules/analytics/analyticsService.js";
...
async function run() {
  ...
  const rows = res.rows;
  const erpDuplicates: Array<{
    numeric: typeof rows[0];
    alphanumeric: typeof rows[0];
  }> = [];
  ...
  for (let i = 0; i < rows.length; i++) {
    for (let j = 0; j < rows.length; j++) {
      ...
      // Handle standard numeric/alphanumeric prefix match pairs
  ...
  try {
    for (const pair of erpDuplicates) {
      ...
      await client.query(
        `DELETE FROM customer_label_assignments WHERE customer_id = $1`,
        [alpha.id]
      );

      // 3. Merge message_automation_customer_events (handle unique constraint)
      await client.query(
        `INSERT INTO message_automation_customer_events (automation_id, customer_id, event_key, last_triggered_run_id, created_at)
         SELECT automation_id, $1::uuid, event_key, last_triggered_run_id, created_at 
      ...
      await client.query(
        `DELETE FROM message_automation_customer_events WHERE customer_id = $1`,
        [alpha.id]
      );
