...
export function CustomerTable({ customers }) {
    ...
    if (!customers.length) {
        return (_jsx("div", { className: "panel table-panel empty-panel", children: _jsx("div", { className: "empty-state", children: "Nenhum cliente encontrado para esse filtro." }) }));
    }
    return (_jsx("div", { className: "panel table-panel", children: _jsx("div", { className: "table-scroll", children: _jsxs("table", { className: "data-table", style: { minWidth: `${tableWidth}px` }, children: [_jsx("colgroup", { children: columns.map((column) => (_jsx("col", { style: { width: `${columnWidths[column.id]}px` } }, column.id))) }), _jsx("thead", { children: _jsx("tr", { children: columns.map((column) => {
                            ...
                            }) }) }), _jsx("tbody", { children: sortedCustomers.map((customer) => (_jsxs("tr", { children: [_jsx("td", { children: _jsxs(Link, { className: "table-link", to: `/clientes/${customer.id}`, children: [_jsx("strong", { children: customer.displayName }), _jsx("span", { children: customer.customerCode })] }) }), _jsx("td", { children: _jsx("span", { className: `status-badge status-${customer.status.toLowerCase()}`, children: statusLabel(customer.status) }) }), _jsx("td", { children: formatDate(customer.lastPurchaseAt) }), _jsx("td", { children: formatDaysSince(customer.daysSinceLastPurchase) }), _jsx("td", { children: customer.totalOrders }), _jsx("td", { children: formatCurrency(customer.avgTicket) }), _jsx("td", { children: formatCurrency(customer.totalSpent) }), _jsx("td", { children: _jsx("div", { className: "tag-row compact", children: customer.labels.length ? (customer.labels.map((label) => (_jsx("span", { className: "tag", style: { background: `${label.color}14`, color: label.color, borderColor: `${label.color}33` }, children: label.name }, label.id)))) : (_jsx("span", { className: "muted-copy", children: "Sem rotulo" })) }) }), _jsx("td", { children: customer.priorityScore.toFixed(1) }), _jsx("td", { children: customer.primaryInsight ?? "Sem alerta" })] }, customer.id))) })] }) }) }));
}
