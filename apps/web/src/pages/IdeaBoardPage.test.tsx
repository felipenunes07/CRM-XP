...
describe("IdeaBoard frontend", () => {
  ...
  it("shows the display-name field only when the author chooses to identify themselves", () => {
    const anonymousMarkup = renderToStaticMarkup(
      <IdeaBoardPageView
        ideas={[]}
        lanes={buildIdeaBoardLanes([])}
        timeline={buildIdeaTimeline([], 4)}
        activeLaneId="ALL"
        selectedIdea={null}
        isCreateModalOpen
        createDraft={{
    ...
    const identifiedMarkup = renderToStaticMarkup(
      <IdeaBoardPageView
        ideas={[]}
        lanes={buildIdeaBoardLanes([])}
        timeline={buildIdeaTimeline([], 4)}
