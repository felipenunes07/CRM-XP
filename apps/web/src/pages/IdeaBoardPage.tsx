...
export function IdeaBoardPage() {
  ...
  const [createDraft, setCreateDraft] = useState<IdeaCreateDraft>(emptyIdeaCreateDraft);
  const [voteDraft, setVoteDraft] = useState<IdeaVoteDraft>(emptyIdeaVoteDraft);
  ...
  const createMutation = useMutation({
    mutationFn: (payload: ReturnType<typeof buildIdeaCreatePayload>) => api.createIdea(token!, payload),
  ...
  const voteMutation = useMutation({
    mutationFn: (payload: ReturnType<typeof buildIdeaVotePayload>) => api.submitIdeaVote(token!, selectedIdeaId!, payload),
  ...
  const deleteMutation = useMutation({
    mutationFn: (ideaId: string) => api.deleteIdea(token!, ideaId),
  ...
  const notifyMutation = useMutation({
    mutationFn: (ideaId: string) => api.notifyIdeaWhatsapp(token!, ideaId),
    onSuccess: () => {
      setNotifyError(null);
  ...
  return (
    <IdeaBoardPageView
      ...
      selectedIdea={selectedIdeaQuery.data ?? null}
      ...
      isIdeaLoading={Boolean(selectedIdeaId) && selectedIdeaQuery.isLoading}
