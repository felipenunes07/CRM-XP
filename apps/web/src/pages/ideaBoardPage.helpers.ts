...
export interface IdeaVoteDraft {
  option: IdeaVoteOption | null;
  comment: string;
}

export interface IdeaTimelinePoint {
  key: string;
  label: string;
  createdCount: number;
  totalCount: number;
...
export const emptyIdeaCreateDraft: IdeaCreateDraft = {
  title: "",
  description: "",
  isAnonymous: true,
  authorDisplayName: "",
...
export const emptyIdeaVoteDraft: IdeaVoteDraft = {
  option: null,
  comment: "",
};
...
function getVoteCounts(idea: Pick<IdeaBoardItem, "voteSummary">) {
  return {
...
export function getIdeaLaneTitle(laneId: IdeaBoardLaneId) {
  return laneBlueprints.find((lane) => lane.id === laneId)?.title ?? laneId;
}

export function buildIdeaCreatePayload(draft: IdeaCreateDraft) {
...
export function buildIdeaVotePayload(draft: IdeaVoteDraft) {
  if (!draft.option) {
...
export function getIdeaVoteDraft(detail?: Pick<IdeaBoardDetail, "currentUserVote"> | null): IdeaVoteDraft {
  return {
...
export function buildIdeaTimeline(ideas: IdeaBoardItem[], days = 10): IdeaTimelinePoint[] {
  const now = new Date();
...
export function formatIdeaStatus(status: "OPEN" | "CLOSED") {
  return status === "OPEN" ? "Votacao aberta" : "Votacao encerrada";
}

export function formatIdeaVoteOption(option: IdeaVoteOption) {
  return ideaVoteOptions.find((item) => item.option === option)?.label ?? option;
}

export function getIdeaVoteRatio(idea: Pick<IdeaBoardItem, "voteSummary">, option: IdeaVoteOption) {
...
export function truncateIdeaCopy(value: string, maxLength = 190) {
