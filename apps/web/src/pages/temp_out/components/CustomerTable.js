...
export function CustomerTable({ customers, selectable = false, selectedIds = [], onSelectedIdsChange, }) {
    ...
    return (_jsx("div", { className: "panel table-panel", children: _jsx("div", { className: "table-scroll", children: _jsxs("table", { className: "data-table", style: { minWidth: `${tableWidth}px` }, children: [_jsxs("colgroup", { children: [selectable && _jsx("col", { style: { width: "50px" } }), localizedColumns.map((column) => (_jsx("col", { style: { width: `${columnWidths[column.id]}px` } }, column.id)))] }), _jsx("thead", { children: _jsxs("tr", { children: [selectable && (_jsx("th", { style: { width: "50px", textAlign: "center", verticalAlign: "middle", padding: "0 10px" }, children: _jsx("input", { type: "checkbox", checked: allSelected, onChange: toggleSelectAll, style: { cursor: "pointer", transform: "scale(1.15)", accentColor: "var(--primary)" } }) })), localizedColumns.map((column) => {
                                ...
                                })] }) }), _jsx("tbody", { children: sortedCustomers.map((customer) => {
                            const isSelected = selectedIds.includes(customer.id);
                            return (_jsxs("tr", { className: isSelected ? "is-selected-row" : "", style: isSelected ? { backgroundColor: "rgba(37, 99, 235, 0.05)" } : undefined, children: [selectable && (_jsx("td", { style: { textAlign: "center", verticalAlign: "middle", padding: "0 10px" }, children: _jsx("input", { type: "checkbox", checked: isSelected, onChange: () => toggleSelectCustomer(customer.id), style: { cursor: "pointer", transform: "scale(1.15)", accentColor: "var(--primary)" } }) })), _jsx("td", { children: _jsxs(Link, { className: "table-link", to: `/clientes/${customer.id}`, children: [_jsx("strong", { children: customer.displayName }), _jsx("span", { children: customer.customerCode }), customer.isAmbassador ? _jsx("small", { className: "table-inline-badge", children: AMBASSADOR_LABEL_NAME }) : null] }) }), _jsx("td", { children: _jsx("span", { className: `status-badge status-${customer.status.toLowerCase()}`, children: statusLabel(customer.status) }) }), _jsx("td", { children: formatDate(customer.lastPurchaseAt) }), _jsx("td", { children: formatDaysSince(customer.daysSinceLastPurchase) }), _jsx("td", { children: customer.totalOrders }), _jsx("td", { children: customer.avgDaysBetweenOrders !== null && customer.avgDaysBetweenOrders !== undefined ? (locale === "zh-CN"
