...
// Find the end of migrations array
const lastBracketIndex = content.lastIndexOf('];');
if (lastBracketIndex === -1) {
  console.error('Error: Could not find the closing bracket of migrations array!');
  process.exit(1);
}

// Find the last backtick before the closing bracket
const lastBacktickIndex = content.lastIndexOf('`', lastBracketIndex);
if (lastBacktickIndex === -1) {
  console.error('Error: Could not find the last backtick!');
  process.exit(1);
}

// Check what is between the last backtick and closing bracket
console.log('Trailing segment:', JSON.stringify(content.substring(lastBacktickIndex, lastBracketIndex + 2)));

const newMigrationSql = `  \`,
  \`
...
const updatedContent = content.substring(0, lastBacktickIndex) + newMigrationSql;
fs.writeFileSync(filePath, updatedContent, 'utf8');
console.log('Successfully appended new migration!');
