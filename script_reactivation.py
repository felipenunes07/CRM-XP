...
new_jsx = """return (
    <div className="page-stack">
      ...
      <section className="panel" style={{ padding: '0', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div className="panel-header" style={{ padding: '1.25rem 1.25rem 1rem 1.25rem', borderBottom: '1px solid var(--line)', background: 'rgba(0,0,0,0.01)' }}>
          <div>
              ...
              return (
                <article key={`${entry.attendant}-${index}`} style={{ borderBottom: '1px solid var(--line)', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ 
                      ...
                      background: isLeader ? 'linear-gradient(90deg, rgba(41, 86, 215, 0.08) 0%, transparent 100%)' : 'transparent',
                      borderLeft: isLeader ? '4px solid var(--accent)' : '4px solid transparent'
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <div style={{ 
                          ...
                          borderRadius: '8px', 
                          background: isLeader ? 'var(--accent)' : 'var(--line)', 
                          color: isLeader ? '#fff' : 'var(--muted)',
                          fontSize: '1rem', fontWeight: 800,
                          boxShadow: isLeader ? '0 0 15px rgba(41, 86, 215, 0.4)' : 'none'
                      }}>
                        {index + 1}
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                          <strong style={{ fontSize: '1.2rem', color: isLeader ? 'var(--accent)' : 'var(--text)' }}>{entry.attendant}</strong>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                       ...
                       <strong style={{ fontSize: '1.25rem', color: isLeader ? 'var(--success)' : 'var(--text)' }}>{formatCurrency(entry.recoveredRevenue)}</strong>
                    </div>
                  </div>
